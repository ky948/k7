...
export const TradingChart: React.FC<TradingChartProps> = ({
  ...
  const currentPriceY = chartHeight - (((asset.price ?? 0) - minPrice) / priceRange) * chartHeight;
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 text-[11px] text-slate-400">
            <span className="flex items-center gap-1">
              <span className="w-2 h-0.5 bg-cyan-400"></span> EMA 9: {asset.ema9 != null ? `$${asset.ema9}` : 'Sync'}
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-0.5 bg-amber-400"></span> EMA 21: {asset.ema21 != null ? `$${asset.ema21}` : 'Sync'}
            </span>
            <span className="flex items-center gap-1 text-slate-300">
              RSI: <span className="font-semibold text-purple-400">{asset.rsi != null ? asset.rsi : 'Sync'}</span>
            </span>
          </div>
      ...
      <div className="px-4 py-1.5 bg-[#0d1322] border-b border-[#19243b] text-[11px] font-mono text-slate-400 flex flex-wrap items-center gap-3">
        {hoveredCandle ? (
          ...
          <>
            ...
            <span>Live Price: <span className="text-slate-100 font-bold">${(asset.price ?? 0).toFixed(2)}</span></span>
            <span>24h High: <span className="text-emerald-400">${(asset.high24h ?? asset.price ?? 0).toFixed(2)}</span></span>
            <span>24h Low: <span className="text-rose-400">${(asset.low24h ?? asset.price ?? 0).toFixed(2)}</span></span>
            <span>VWAP: <span className="text-cyan-400">{asset.vwap != null ? `$${asset.vwap.toFixed(2)}` : 'Live Ticker'}</span></span>
            <span>ATR: <span className="text-amber-400">{asset.atr != null ? `$${asset.atr.toFixed(2)}` : 'Calibrating'}</span></span>
          </>
        )}
      </div>
